import React from 'react'
import Modal from './Modal'

export default function OfficeOrder() {
    return (
        <div className='max-w-screen-lg mx-auto bg-white'>
            <div className='flex items-center place-content-center space-x-8 my-4'>
                <img className='w-24' src="http://www.iitrpr.ac.in/sites/default/files/image.jpg" alt="" />
                <div className='flex-col '>
                    <h1 className='text-2xl'>Indian Institute of Technology Ropar</h1>
                    <p>Rupnagar, Punjab 140001 </p>
                </div>
            </div>
            <div className='flex place-content-around space-x-32 mb-4'>
                <p>F.No. IITRPR/ACAD/4740</p>
                <p>Date:27.07.2020</p>
            </div>
            <div className='flex justify-center'>

                <h2 className='text-xl font-extrabold underline'>Office Order</h2>
            </div>
            <div className='border-t-4 border-gray-300 my-2 flex'>
                <p className='m-4 text-lg'>
                    <p className='mb-4 mt-8 font-semibold'>
                        Subject: Approval of Travel Allowance
                    </p>

                    <p className='mb-4'>
                        This is to inform that Applicant's application for Travel Allowance for the LTC Application <span className='font-bold'> [Insert Year] </span> has been approved by the management.
                    </p>

                    <p>
                        The details of LTC are as follows:
                    </p>
                    <div className='mb-32'>
                        <li>
                            Destination of travel: [Insert Destination]
                        </li>
                        <li>
                            Mode of travel: [Insert Mode of Travel]
                        </li>
                        <li>
                            Duration of travel: [Insert Duration]
                        </li>
                        <li>
                            Names of accompanying family members (if any): [Insert Names of Family Members]
                        </li>

                    </div>

                    <p>
                        Copy To:
                        <div className='mb-4 mt-2'>
                            <li>
                                Dean (Faculty Affairs & Administration)
                            </li>
                            <li>
                                HOD
                            </li>
                            <li>
                                Applicant
                            </li>
                            <li>
                                Registrar (Accounts)
                            </li>

                        </div>
                    </p>
                    <div className='flex justify-end items-end'>
                        <div className='fex-col'>
                            <img className='w-300 h-150' src="data:image/png;base64," alt="Sign" />
                            <p className='px-32'>
                                Registrar
                            </p>
                        </div>
                    </div>
                </p>
            </div>
        </div>
    )
}
